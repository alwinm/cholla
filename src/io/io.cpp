...
void Write_Slices_HDF5_Field(Header H, hid_t file_id, Real* host_hdf5_buffer,
			     Real* device_hdf5_buffer,
			     Real *device_source_buffer,
                             const char *name)
{
  ...
  z_slice = H.nz / 2;
  #ifdef MPI_CHOLLA
  x_slice = nx_global / 2 - nx_local_start + H.n_ghost;
  y_slice = ny_global / 2 - ny_local_start + H.n_ghost;
  z_slice = nz_global / 2 - nz_local_start + H.n_ghost;
  #endif
  ...
  #ifdef MPI_CHOLLA
  yz_check = (x_slice >= H.n_ghost) && (x_slice < nx_local + H.n_ghost);
  xz_check = (y_slice >= H.n_ghost) && (y_slice < ny_local + H.n_ghost);
  xy_check = (z_slice >= H.n_ghost) && (z_slice < nz_local + H.n_ghost);
  #endif
  ...
  if (yz_check) {
    ...
    Device_To_HDF5_Buffer(1, H.ny_real, H.nz_real, x_slice, H.n_ghost, H.n_ghost, H.nx, H.ny, host_hdf5_buffer,
                          device_hdf5_buffer, device_source_buffer);
    status = Write_HDF5_Dataset(file_id, dataspace_id_yz, host_hdf5_buffer, name_yz.data());
  }
  ...
  if (xz_check) {
    ...
    Device_To_HDF5_Buffer(H.nx_real, 1, H.nz_real, H.n_ghost, y_slice, H.n_ghost, H.nx, H.ny, host_hdf5_buffer,
                          device_hdf5_buffer, device_source_buffer);
    status = Write_HDF5_Dataset(file_id, dataspace_id_xz, host_hdf5_buffer, name_xz.data());
  }
  ...
  if (xy_check) {
    ...
    Device_To_HDF5_Buffer(H.nx_real, H.ny_real, 1, H.n_ghost, H.n_ghost, z_slice, H.nx, H.ny, host_hdf5_buffer,
                          device_hdf5_buffer, device_source_buffer);
    status = Write_HDF5_Dataset(file_id, dataspace_id_xy, host_hdf5_buffer, name_xy.data());
  }
...
{
  ScopedTimer timer("Write_Slices_HDF5");
  if (H.nx == 1 || H.ny == 1 || H.nz == 1) {
    chprintf("Slice write only works for 3D data.\n");
    return;
  }
 
  int minimum_axis = std::min(H.nx_real, H.ny_real);
  minimum_axis = std::min(minimum_axis, H.nz_real);

  size_t buffer_size = (H.nx_real * H.ny_real * H.nz_real) / minimum_axis;
  printf("Buffer size: %d \n", buffer_size);
  // Allocate device-side and host-side hdf5 buffers
  Real *host_hdf5_buffer = (Real *) malloc(buffer_size * sizeof(Real));
  cuda_utilities::DeviceVector<Real> static device_hdf5_vector{buffer_size};
  Real* device_hdf5_buffer = device_hdf5_vector.data();
  
  // Write slices for each field
  Write_Slices_HDF5_Field(H, file_id, host_hdf5_buffer, device_hdf5_buffer, C.d_density, "d");
  Write_Slices_HDF5_Field(H, file_id, host_hdf5_buffer, device_hdf5_buffer, C.d_momentum_x, "mx");
  Write_Slices_HDF5_Field(H, file_id, host_hdf5_buffer, device_hdf5_buffer, C.d_momentum_y, "my");
  Write_Slices_HDF5_Field(H, file_id, host_hdf5_buffer, device_hdf5_buffer, C.d_momentum_z, "mz");
  Write_Slices_HDF5_Field(H, file_id, host_hdf5_buffer, device_hdf5_buffer, C.d_Energy, "E");

  #ifdef DE
  Write_Slices_HDF5_Field(H, file_id, host_hdf5_buffer, device_hdf5_buffer, C.d_GasEnergy, "GE");
  #endif  // DE
  #ifdef SCALAR
    #ifdef BASIC_SCALAR
  Write_Slices_HDF5_Field(H, file_id, host_hdf5_buffer, device_hdf5_buffer, C.d_basic_scalar, "scalar");
#endif //BASIC_SCALAR
  #endif  // SCALAR
  free(host_hdf5_buffer);
}
#endif  // HDF5
