...
void Grid3D::Write_Grid_HDF5(hid_t file_id)
{
  ...
  if (H.nx > 1 && H.ny == 1 && H.nz == 1) {
    ...
    for (int s = 0; s < NSCALARS; s++) {
      ...
      // TODO: If there is a test case for regression testing NSCALARS > 1 this
      // substitution can be attempted. Write_HDF5_Field_1D_CPU(H, file_id,
  ...
  if (H.nx > 1 && H.ny > 1 && H.nz > 1) {
    ...
    size_t buffer_size;
  // Need a larger device buffer for MHD. In the future, if other fields need a
  // larger device buffer, choose the maximum of the sizes. If the buffer is too
  // large, it does not cause bugs (Oct 6 2022)
  #ifdef MHD
    buffer_size = (nx_dset + 1) * (ny_dset + 1) * (nz_dset + 1);
  #else
    buffer_size = nx_dset * ny_dset * nz_dset;
    ...
    // Create the data space for the datasets (note: WriteHDF5Field3D creates
    // its own dataspace, does not use the shared one)
    dims[0]      = nx_dset;
    dims[1]      = ny_dset;
    dims[2]      = nz_dset;
    ...
    // Free the dataspace id
    status = H5Sclose(dataspace_id);
    // CudaSafeCall(cudaFree(device_dataset_buffer));// No longer needed because
    // devicevector frees when it should
  }
  free(dataset_buffer);
}
#endif  // HDF5
...
void Write_Slices_HDF5_Field(Header H, hid_t file_id, std::vector<Real> host_hdf5_vector,
                             cuda_utilities::DeviceVector<Real> device_hdf5_vector, Real *device_source_buffer,
                             const char *name)
{
  ...
  if (xz_check) {
    ...
    Device_To_HDF5_Buffer(H.nx_real, 1, H.nz_real, H.n_ghost, y_slice, H.n_ghost, H.nx, H.ny, host_hdf5_vector.data(),
                          device_hdf5_vector.data(), device_source_buffer);
  ...
  if (xy_check) {
    ...
    Device_To_HDF5_Buffer(H.nx_real, H.ny_real, 1, H.n_ghost, H.n_ghost, z_slice, H.nx, H.ny, host_hdf5_vector.data(),
                          device_hdf5_vector.data(), device_source_buffer);
...
void Grid3D::Write_Slices_HDF5(hid_t file_id)
{
  ...
  // Allocate device-side and host-side hdf5 buffers encapsulated in vectors that will free themselves
  std::vector<Real> host_hdf5_vector{buffer_size};
  cuda_utilities::DeviceVector<Real> static device_hdf5_vector{buffer_size};

  Write_Slices_HDF5_Field(H, file_id, host_hdf5_vector, device_hdf5_vector, C.d_density, "d");
  Write_Slices_HDF5_Field(H, file_id, host_hdf5_vector, device_hdf5_vector, C.d_momentum_x, "mx");
  Write_Slices_HDF5_Field(H, file_id, host_hdf5_vector, device_hdf5_vector, C.d_momentum_y, "my");
  Write_Slices_HDF5_Field(H, file_id, host_hdf5_vector, device_hdf5_vector, C.d_momentum_z, "mz");
  Write_Slices_HDF5_Field(H, file_id, host_hdf5_vector, device_hdf5_vector, C.d_Energy, "E");
  #ifdef DE
  Write_Slices_HDF5_Field(H, file_id, host_hdf5_vector, device_hdf5_vector, C.d_GasEnergy, "GE");
  #endif  // DE
  #ifdef SCALAR
    #ifdef BASIC_SCALAR
  Write_Slices_HDF5_Field(H, file_id, host_hdf5_vector, device_hdf5_vector, C.d_basic_scalar, "scalar");
    #endif BASIC_SCALAR
  #endif  // SCALAR
}
#endif  // HDF5
