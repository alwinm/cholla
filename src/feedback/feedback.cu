#if defined(FEEDBACK) && defined(PARTICLES_GPU) && defined(PARTICLE_AGE) && \
...
inline __device__ Real Calc_Timestep(Real gamma, Real* density,
  Real* momentum_x, Real* momentum_y, Real* momentum_z, Real* energy,
  int index, Real dx, Real dy, Real dz)
{
  ...
  Real P  = fmax((energy[index] - 0.5 * dens * (vx * vx + vy * vy + vz * vz))
                 * (gamma - 1.0), TINY_NUMBER);
  Real cs = sqrt(gamma * P * d_inv);
  return fmax(fmax((fabs(vx) + cs) / dx, (fabs(vy) + cs) / dy)
                  , (fabs(vz) + cs) / dz);
}
...
/** the prescription for dividing a scalar quantity between 3x3x3 cells is done
   ...
   cells/direction.
  */
inline __device__ Real frac(int i, Real dx)
{
  return (-0.5 * i * i - 0.5 * i + 1 + i * dx) * 0.5;
}


inline __device__ Real d_fr(int i, Real dx)
{
  return (dx > 0.5) * i * (1 - 2 * dx) + ((i + 1) * dx + 0.5 * (i - 1)) -
         3 * (i - 1) * (i + 1) * (0.5 - dx);
}
...
inline __device__ Real GetAverageDensity(Real* density, int xi, int yi, int zi,
                                  int nx_grid, int ny_grid, int n_ghost)
{
  Real d_average = 0.0;
  for (int i = -1; i < 2; i++) {
    for (int j = -1; j < 2; j++) {
      for (int k = -1; k < 2; k++) {
        d_average += density[(xi + n_ghost + i) + (yi + n_ghost + j) * nx_grid +
                             (zi + n_ghost + k) * nx_grid * ny_grid];
      }
    }
  }
  return d_average / 27;
...
void feedback::initState(struct parameters* P)
{
  ...
  if (not snr_filename.empty()) {
    ...
    CHECK(cudaMemcpy(dev_snr, snr.data(), snr.size() * sizeof(Real),
 ...
 */
 void feedback::initWindState(struct parameters* P)
 {
   chprintf("initWindState start\n");
  ...
  */
__device__ Real GetWindFlux(Real t, Real* dev_sw_p, Real sw_dt, Real t_start,
  Real t_end)
{
  if (t < t_start || t >= t_end) return 0;

  int index = (int)((t - t_start) / sw_dt);
  Real log_p_dynes = dev_sw_p[index] +
    (t - index * sw_dt) * (dev_sw_p[index + 1] - dev_sw_p[index]) / sw_dt;
 ...
 */
 __device__ Real GetWindPower(Real t, Real* dev_sw_e, Real sw_dt, Real t_start,
   Real t_end)
{
  if (t < t_start || t >= t_end) return 0;
  ...
  Real e = pow(10, log_e) / (MASS_UNIT * VELOCITY_UNIT * VELOCITY_UNIT) *
           TIME_UNIT / S_99_TOTAL_MASS;
  return e;
}
                          ...
                          Real t_end)
{
  ...
  return dev_snr[index] +
         (t - index * snr_dt) * (dev_snr[index + 1] - dev_snr[index]) / snr_dt;
}
...
__device__ Real Apply_Resolved_SN(Real pos_x, Real pos_y, Real pos_z,
                  Real xMin, Real yMin, Real zMin, Real dx, Real dy, Real dz,
                  int nx_g, int ny_g, int n_ghost, int n_cells, Real gamma,
                  Real* conserved_device, short time_direction,
...
__device__ Real Apply_Unresolved_SN_old(Real pos_x, Real pos_y, Real pos_z,
                  Real xMin, Real yMin, Real zMin, Real dx, Real dy, Real dz,
                  int nx_g, int ny_g, int n_ghost, int n_cells, Real gamma,
                  Real* conserved_device, short time_direction,
                  ...
                  Real feedback_energy, int indx_x, int indx_y, int indx_z)
{
  ...
  // Precompute total magnitude for normalization
  for (int i = -1; i < 2; i++) {
    for (int j = -1; j < 2; j++) {
      for (int k = -1; k < 2; k++) {
  ...
  for (int i = -1; i < 2; i++) {
    for (int j = -1; j < 2; j++) {
      for (int k = -1; k < 2; k++) {
        ...
        Real d  = sqrt(x_frac*x_frac + y_frac*y_frac + z_frac*z_frac)
                        / mag * feedback_density;
        Real e  = sqrt(x_frac*x_frac + y_frac*y_frac + z_frac*z_frac)
                        / mag * feedback_energy;
...
__device__ Real Apply_Unresolved_SN(Real pos_x, Real pos_y, Real pos_z,
				    ...
				    Real feedback_energy, int indx_x, int indx_y, int indx_z)
{
  ...
  for (int i = -1; i < 2; i++) {
	...
	Real square_mag = x_frac*x_frac + y_frac*y_frac + z_frac*z_frac;
  ...
  // Injected kinetic energy = (p^2)/2m = (x_frac * feedback_momentum + time_direction * d * vel_x)^2 = (x_frac * feedback_momentum)^2 + (d*vel_x)^2 + x_frac * feedback_momentum * time_direction * d * vel_x = sum sqr (x_frac * feedback_momentum) + sum sqr(d) * vel_x^2 + sum x_frac * d * vel_x * feedback_momentum * time_direction


  // Compute total kinetic energy injected by the bullets agnostic of time_direction because it cancels in p^2 and abs(d)

  Real kinetic_energy_injected = 0;
  for (int i = -1; i < 2; i++) {
    for (int j = -1; j < 2; j++) {
      for (int k = -1; k < 2; k++) {
        ...
        Real d  = sqrt(x_frac*x_frac + y_frac*y_frac + z_frac*z_frac)
                        / mag * feedback_density;
	// SNe momentum + momentum from particle mass loss
  ...
  // Feedback_density has time_direction baked in so
  // Feedback_density, particle_energy, and feedback_energy all have time_direction baked in
  const Real particle_energy = 0.5 * feedback_density * (vel_x*vel_x + vel_y*vel_y + vel_z*vel_z);
  feedback_energy += particle_energy;

  Real kinetic_factor = 1.0;
  Real thermal_energy_injection = fabs(feedback_energy) - fabs(kinetic_energy_injected);
  ...
  for (int i = -1; i < 2; i++) {
	...
	// SNe momentum + momentum from particle mass loss
        Real px = kinetic_factor * (x_frac * feedback_momentum + d * vel_x);
        Real py = kinetic_factor * (y_frac * feedback_momentum + d * vel_y);
        Real pz = kinetic_factor * (z_frac * feedback_momentum + d * vel_z);
...
__device__ Real Apply_Wind(Real pos_x, Real pos_y, Real pos_z, Real xMin,
                  Real yMin, Real zMin, Real dx, Real dy, Real dz, int nx_g,
                  int ny_g, int n_ghost, int n_cells, Real gamma,
                  Real* conserved_device, short time_direction,
                  Real feedback_density, Real feedback_momentum,
  ...
  for (int i = -1; i < 2; i++) {
    for (int j = -1; j < 2; j++) {
      for (int k = -1; k < 2; k++) {
        ...
        Real py = y_frac * feedback_momentum;
        Real pz = z_frac * feedback_momentum;
        f_dens   = sqrt(x_frac*x_frac + y_frac*y_frac + z_frac*z_frac)
                   / mag * feedback_density;
        f_energy = sqrt(x_frac*x_frac + y_frac*y_frac + z_frac*z_frac)
                   / mag * feedback_energy;
        ...
        */
        ...
        /*
        if (threadIdx.x == 0 && i == -1 && j == 0 && k == 1) {
          if (time_direction > 0) {
            kernel_printf("~~~~  WF: n_s: %d, id: %lld, l: %d [%d, %d, %d], fe: %.5e, px: %.5e, py: %.5e, pz: %.5e, %.5e\n",
                n_step, id, loop, i, j, k, f_energy, px, py, pz, fd);
          } else {
            kernel_printf("~~~~ -WF: n_s: %d, id: %lld, l: %d [%d, %d, %d], fe: %.5e, px: %.5e, py: %.5e, pz: %.5e, %.5e\n",
                n_step, id, loop, i, j, k, f_energy, px, py, pz, fd);
          }
...
__device__ void SN_Feedback(Real pos_x, Real pos_y, Real pos_z,
  ...
  Real* prev_dens, short time_direction, Real* s_info, Real* conserved_dev,
  Real gamma, int loop, int indx_x, int indx_y, int indx_z)
{
  ...
  int N = GetNumberOfSNeForCluster(average_num_sn, n_step, id_dev[gtid])
                             * time_direction;
  /*
  if (gtid == 0) {
...
__device__ void Wind_Feedback(Real pos_x, Real pos_y, Real pos_z, Real age,
  Real* mass_dev, part_int_t* id_dev, Real xMin, Real yMin, Real zMin,
  Real xMax, Real yMax, Real zMax, Real dx, Real dy, Real dz, int nx_g,
  int ny_g, int nz_g, int n_ghost, int n_step, Real t, Real dt, Real* dti,
  ...
  Real gamma, int loop, int indx_x, int indx_y, int indx_z)
{
  ...
  feedback_energy = GetWindPower(age, dev_sw_e, sw_dt, time_sw_start, time_sw_end);
  feedback_density = GetWindMass(feedback_momentum, feedback_energy);
  ...
  local_dti = Apply_Wind( pos_x,  pos_y,  pos_z, xMin,  yMin,  zMin, dx,  dy,
    dz, nx_g,  ny_g,  n_ghost,  n_cells, gamma, conserved_dev, time_direction,
    feedback_density, feedback_momentum, feedback_energy, n_step, id_dev[gtid],
    loop, indx_x, indx_y, indx_z);
...
__device__ void Cluster_Feedback_Helper( part_int_t n_local,
  ...
  int n_ghost, int n_step, Real t, Real dt, Real* dti, Real* dev_snr,
  Real snr_dt, Real time_sn_start, Real time_sn_end, Real* prev_dens,
  Real* dev_sw_p, Real* dev_sw_e, Real sw_dt, Real time_sw_start,
  Real time_sw_end, short time_direction, Real* s_info, Real* conserved_dev,
  Real gamma, int loop )
{
  ...
  if (time_direction > 0) {
    ...
    if (is_wd_feedback) Wind_Feedback(pos_x, pos_y, pos_z, age, mass_dev, id_dev,
                          xMin, yMin, zMin, xMax, yMax, zMax, dx, dy, dz, nx_g,
                          ny_g, nz_g, n_ghost, n_step, t, dt, dti, dev_sw_p,
...
__global__ void Cluster_Feedback_Kernel(
    ...
    Real* info, Real* density, Real gamma, Real* prev_dens,
    short time_direction, Real* dev_snr, Real snr_dt, Real time_sn_start,
    Real time_sn_end, Real* dev_sw_p, Real* dev_sw_e, Real sw_dt,
    Real time_sw_start, Real time_sw_end, int n_step, int loop)
{
...
__global__ void Adjust_Cluster_Mass_Kernel(part_int_t n_local, Real* pos_x_dev,
...
__global__ void Set_Ave_Density_Kernel(
  part_int_t n_local, Real* pos_x_dev, Real* pos_y_dev, Real* pos_z_dev,
  Real* mass_dev, Real* age_dev, part_int_t* id_dev, Real xMin, Real yMin,
  Real zMin, Real xMax, Real yMax, Real zMax, Real dx, Real dy, Real dz,
  int nx_g, int ny_g, int nz_g, int n_ghost, Real t, Real dt, Real* density,
  Real* dev_snr, Real snr_dt, Real time_sn_start, Real time_sn_end,
  Real time_sw_start, Real time_sw_end, int n_step)
{
  int tid = threadIdx.x;
  int gtid = blockIdx.x * blockDim.x + tid;
  // Bounds check on particle arrays
...
Real feedback::Cluster_Feedback(Grid3D& G, FeedbackAnalysis& analysis)
{
  ...
  if (G.Particles.n_local > 0) {
    ...
    hipLaunchKernelGGL(
      Set_Ave_Density_Kernel, ngrid, TPB_FEEDBACK, 0, 0, G.Particles.n_local,
      G.Particles.pos_x_dev, G.Particles.pos_y_dev, G.Particles.pos_z_dev,
      G.Particles.mass_dev, G.Particles.age_dev, G.Particles.partIDs_dev,
      G.H.xblocal, G.H.yblocal, G.H.zblocal, G.H.xblocal_max, G.H.yblocal_max,
      G.H.zblocal_max, G.H.dx, G.H.dy, G.H.dz, G.H.nx, G.H.ny, G.H.nz,
      G.H.n_ghost, G.H.t, G.H.dt, G.C.d_density, dev_snr, snr_dt,
      time_sn_start, time_sn_end, time_sw_start, time_sw_end, G.H.n_step);
  }
  ...
  do {
    ...
    if (G.Particles.n_local > 0) {
	  ...
	  G.Particles.mass_dev,
          ...
          G.H.dz, G.H.nx, G.H.ny, G.H.nz, G.H.n_ghost, G.H.t, G.H.dt, d_dti,
          d_info, G.C.d_density, gama, d_prev_dens, time_direction, dev_snr,
          snr_dt, time_sn_start, time_sn_end, dev_sw_p, dev_sw_e, sw_dt,
  ...
  if (G.Particles.n_local > 0) {
    hipLaunchKernelGGL(
      ...
      G.H.xblocal, G.H.yblocal, G.H.zblocal, G.H.xblocal_max, G.H.yblocal_max,
      G.H.zblocal_max, G.H.dx, G.H.dy, G.H.dz, G.H.nx, G.H.ny, G.H.nz,
      G.H.n_ghost, G.H.n_step, G.H.t, G.H.dt, dev_snr, snr_dt, time_sn_start,
      time_sn_end, dev_sw_p, dev_sw_e, sw_dt, time_sw_start,
      time_sw_end);
  }
  /*
  ...
  if (G.Particles.n_local > 0) {
    CHECK(cudaMemcpy(&h_info, d_info, FEED_INFO_N * sizeof(Real),
  ...
  #ifndef NO_WIND_FEEDBACK
  chprintf(
      "    wind momentum: %.5e S.M. km/s,  cumulative: %.5e S.M. km/s\n",
      info[i_WIND_MOMENTUM] * VELOCITY_UNIT / 1e5,
      analysis.totalWindMomentum * VELOCITY_UNIT / 1e5);
  chprintf(
      "    wind energy  : %.5e erg,  cumulative: %.5e erg\n",
