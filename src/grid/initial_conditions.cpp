...
void Grid3D::Set_Domain_Properties(struct parameters P)
{
  ...
  // lim + dx * i ensures that each rank agrees on the x value of cell i 
  H.xblocal = H.xbound + (P.xlen/nx_param) * ((Real) nx_local_start);
  H.xblocal = H.ybound + (P.ylen/ny_param) * ((Real) ny_local_start);
  H.xblocal = H.zbound + (P.zlen/nz_param) * ((Real) nz_local_start);

#endif

  // Both MPI and non-mpi set dx = Global Length / Global nx 
  // Then domlen_x = dx * (num cells)
  // Then ignored dimension domlen_I = Ilen / (num cells x)
  // Then ignored dimension dI = domlen_I
